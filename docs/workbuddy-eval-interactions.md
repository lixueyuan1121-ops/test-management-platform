...
2026-09-14 模型选择修复：检查 macOS WorkBuddy 5.5.6 实际下拉菜单，确认 `GLM-5.3` 与 `GLM-5.3-Flash` 是独立选项，整行还包含倍率/优惠文字。对照本机客户端组件确认名称使用 `cr-model-selector__item-name`，列表选项全部挂载在滚动容器中。回归在合成 Chromium 页面中验证名称与附加信息分离、大小写、屏外项、隐藏同名项、异步加载、旧选择器、缺失/禁用/歧义、回读失败和发送拦截；另验证错误码回写去重。此次本机未开放 CDP 调试端口，未宣称执行器已在真实客户端完成模型切换或模型对话。
