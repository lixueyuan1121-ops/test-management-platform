...
- ✅ **适用(新增)**:被测系统是**客户端内动态签名鉴权、但签名算法已知**的——典型如 **namisoa/纳米Work、claw/安全龙虾**。只要拿到其签名脚本(如 Postman pre-request 里的 `signApp`/`signWeb`),执行机就能按同样算法**每请求现算** `sign`/`access-token`/`timestamp`/`signnonce`,在客户端外复现鉴权。配置方式:api 环境「鉴权方式」选 **客户端动态签名**(`auth_type=sign`),填 `scheme`(如 `namisoa`)+ 平台/凭据/mid/app_name 等(见 §① )。
  - 前提:base_url 必须是**稳定可达地址**(不是一次性会话子域名);签名算法未变。若被测端改了签名逻辑,需同步更新执行机 `api-signers.mjs` 的对应 scheme。
- ❌ **仍不适用**:签名算法**拿不到 / 无法复现**,或 base_url 是**一次性会话哈希子域名**(`https://<vm_id>.work.n.cn`,每人/每会话不同、会过期)的场景。此时抓真实请求头原样重放几秒即失效(`4001`),**无论怎么填固定 token 都会很快失效**——改用 **gui / e2e 用例**(在客户端界面操作、断言界面结果,鉴权/签名/vm_id 全由客户端自理)。

> 判断口诀:**接口能不能在 Postman/curl 里稳定调通(含用签名脚本现算鉴权)?** 能 → api 用例;不能(只有客户端内才调得动、签名不可复现)→ gui/e2e 用例。
>
> 提示:项目**配好 api 契约 + 签名鉴权**后,AI 生成会正常产出 api 用例;**没配契约时**仍会把本可判 api 的验证点自动改判 gui/e2e(见下方「AI 生成」)。
