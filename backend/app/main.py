...
def init_db() -> None:
    ...
    ensure_exec_run_release_column()
    ensure_exec_run_retry_columns()
    ensure_testcase_requirement_column()
    ...
    ensure_rts_recommendation_table()
    ensure_eval_run_history_table()
    ensure_eval_task_status_enum()
    ensure_eval_run_status_enum()
    ensure_perf_indexes()
    ...
    ensure_run_tracking_columns()
    ensure_eval_task_tables()
    ensure_perf_set_thresholds_column()
    ensure_platform_columns()
    ensure_runner_device_capabilities()
    ensure_runner_device_runner_kind()
