...
4. 内容结构:
   ...
   <h2>失败根因深挖</h2> 挑代表性不通过/低分用例,结合思考过程与工具调用**追出真正的根因**——
     是路径/依赖/语法错误反复出现?多轮调参没收敛?工具选择或调用顺序不当?别停在「答错了」这层,
     要说清**为什么会错**。每个根因都给出具体、可落地的改进建议(该补什么工具、prompt 该怎么改、
     预设该怎么调),不空谈。引用用例标题,给出证据(哪步工具、报错什么)。
   <h2>工具使用质量</h2> 从工具调用序列看:工具选择是否合理、有无多余或缺失调用、调用顺序是否高效。
     **特别注意:某题没调工具或少调工具、结果却达成了,这不是缺点、不减分**——反而要深挖它为何能不靠工具
     一步到位(基座强?prompt 好?推理充分?),把这一点提炼成别的产品可借鉴的经验。过程数据不足时明确说明数据有限。
   ...
   <h2>产品横向对比</h2> 【仅当素材含多个「产品」时才输出本节,否则整节省略】用 <table> 按产品汇总
     ...
     **纠偏:工具调用更少、或耗时更短、但结果相当甚至更好,是优势不是短板,要向它学习它靠什么做到的**——
     不得因为工具调用少就判它弱。各产品模型可能不同,对比整体表现,如实说明不臆断。
   <h2>亮点</h2> 值得肯定的表现。
...
8. 注意:素材里出现「【评测系统截断:…】」是评测系统截断展示,不是被测模型输出中断,不要当缺陷写。"""
...
def _validate_script(script, valid_keys: set[str] | None = None) -> tuple[list, str | None]:
    ...
    至少含一个 assert_text/assert_visible/assert_absent(否则无判定依据)。任一不满足 → 返回错误(调用方降级 manual)。
