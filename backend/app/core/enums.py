...
# 注:设备「能跑功能/测评」不再是静态枚举/配置——改为运行时感知(设备实际在拉 exec-queue 还是
# eval-queue 决定),见 runner_device.last_exec_at/last_eval_at 与 dispatcher。展示用 kind 字符串
# "func"/"eval"(与看板 active_runs.kind 同源),队列/心跳用 "exec"/"eval"(与队列端点同源)。
