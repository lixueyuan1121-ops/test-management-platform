...
class EvalTask(Base):
    ...
    # 综合评价在线短链码:综合评价 done 时生成一次(稳定),对外匿名访问 /r/<code> 渲染 HTML 报告。
    # 一条龙生成完毕后经推推把短链推给人(base=PLATFORM_BASE_URL)。NULL=尚未生成过评价。
    summary_share_code: Mapped[str | None] = mapped_column(String(16), nullable=True, unique=True)
