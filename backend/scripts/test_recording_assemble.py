...
from app.services.recording import assemble_recording_steps, to_camel, _new_key_name


def test_reuse_existing_key():
    # 候选命中已注册 key → 复用 key 名;并把本次候选并入(供 apply_heal_items 合并/补 xpath 等)。
    idx = {"testid nav-agents": "navAgents"}
...
def test_edge_cases():
    ...
    steps, new, _exp = assemble_recording_steps([{"action": "click", "candidates": [{"by": "css"}]}, None], {})
    assert steps == [{"action": "connect", "target": {}, "args": {}, "desc": "连接被测客户端"}], steps
    assert new == []
    print("OK edge_cases")
