...
const permittedGroups = computed(() => visibleNavigation({ admin: auth.isPlatformAdmin, automationTools: auth.canUseAutomationTools, reports: showMyReports.value }))
const groups = computed(() => visibleNavigation({ admin: auth.isPlatformAdmin, automationTools: auth.canUseAutomationTools, reports: showMyReports.value, search: navSearch.value }))
