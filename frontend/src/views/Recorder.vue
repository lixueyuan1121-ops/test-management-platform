<template>
  <div class="recorder">
    <el-card>
      ...
      <div v-if="!recording && steps.length" class="save-bar">
...
const ACT_LABEL = { click: '点击', fill: '输入', assert: '断言' }
...
function startPoll(id) {
  ...
  const poll = async () => {
    ...
    try {
      ...
      if (r.status === 'stopped' || r.status === 'done') {
...
async function onStart() {
  ...
  starting.value = true
  try {
    const res = await startRecord({ project_id: pid.value, sub_product: subProduct.value, runner: runner.value })
...
async function onStop() {
  ...
  try {
    ...
    ElMessage.success(`已停止,共录 ${steps.value.length} 步,核对后保存为用例`)
  } catch {
