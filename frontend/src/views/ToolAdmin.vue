<template>
  <div class="tool-admin functional-workspace">
    <WorkspacePage title="工具配置">
      <template #actions>
        <el-button v-if="activeTab === 'tools'" :disabled="busy || catError || catLoading" type="primary" size="small" @click="openToolCreate">新建工具</el-button>
        <el-button v-else :disabled="busy" type="primary" size="small" @click="openCatCreate">新建分类</el-button>
      </template>
    ...
    <section v-show="activeTab === 'categories'">
      <el-result v-if="catError" icon="error" title="分类加载失败"><template #extra><el-button @click="loadCats">重试分类</el-button></template></el-result>
      <el-table v-else :data="categories" v-loading="catLoading" size="small">
        ...
        <el-table-column label="操作" width="160">
          <template #default="{ row }">
            <el-button link type="primary" :disabled="busy" @click="openCatEdit(row)">编辑</el-button>
            <el-button link type="danger" :disabled="busy" @click="onCatDel(row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </section>
    ...
    <section v-show="activeTab === 'tools'">
        <el-alert v-if="catError" title="分类加载失败" type="warning" :closable="false"><el-button link @click="loadCats">重试分类</el-button></el-alert>
        <div class="header">
          <div>
      ...
      <el-result v-if="toolError" icon="error" title="工具加载失败"><template #extra><el-button @click="loadTools">重试工具</el-button></template></el-result>
      <el-table v-else :data="tools" v-loading="toolLoading" size="small">
        ...
        <el-table-column label="操作" width="200">
          <template #default="{ row }">
            <el-button link :disabled="busy" :type="row.status === 'online' ? 'warning' : 'success'" @click="onToggle(row)">{{ row.status === 'online' ? '下线' : '上线' }}</el-button>
            <el-button link type="primary" :disabled="busy || catError || catLoading" @click="openToolEdit(row)">编辑</el-button>
            <el-button link type="danger" :disabled="busy" @click="onToolDel(row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </section>
    ...
    <!-- 分类对话框 -->
    <el-dialog v-if="catDialog.visible" v-model="catDialog.visible" :show-close="!catDialog.saving" :close-on-click-modal="!catDialog.saving" :close-on-press-escape="!catDialog.saving" :title="catDialog.id ? '编辑分类' : '新建分类'" width="400px">
      <el-form label-width="80px" :disabled="catDialog.saving">
      ...
      <template #footer>
        <el-button :disabled="catDialog.saving" @click="catDialog.visible = false">取消</el-button>
        <el-button type="primary" :loading="catDialog.saving" @click="submitCat">保存</el-button>
      </template>
    </el-dialog>

    <!-- 工具对话框 -->
    <el-dialog v-if="toolDialog.visible" v-model="toolDialog.visible" :show-close="!toolDialog.saving" :close-on-click-modal="!toolDialog.saving" :close-on-press-escape="!toolDialog.saving" :title="toolDialog.id ? '编辑工具' : '新建工具'" width="560px">
      <el-form :model="toolForm" :disabled="toolDialog.saving" label-width="90px">
      ...
      <template #footer>
        <el-button :disabled="toolDialog.saving" @click="toolDialog.visible = false">取消</el-button>
...
const catError = ref(false), toolError = ref(false), busy = ref(false)
let catVersion = 0, toolVersion = 0, disposed = false

onMounted(async () => { await loadCats(); if (!disposed) await loadTools() })
onBeforeUnmount(() => { disposed = true; ++catVersion; ++toolVersion })

async function loadCats() {
  const version = ++catVersion
  catLoading.value = true; catError.value = false
  try {
    const data = await listCategories(true)
    if (!disposed && version === catVersion) categories.value = data
  } catch { if (!disposed && version === catVersion) { categories.value = []; catError.value = true } }
  finally { if (!disposed && version === catVersion) catLoading.value = false }
}
async function loadTools() {
  const version = ++toolVersion
  toolLoading.value = true; toolError.value = false; tools.value = []
  try {
    const data = await listTools({ category_id: toolCatFilter.value || undefined })
    if (!disposed && version === toolVersion) tools.value = data
  } catch { if (!disposed && version === toolVersion) toolError.value = true }
  finally { if (!disposed && version === toolVersion) toolLoading.value = false }
}
...
async function submitCat() {
  if (catDialog.saving || busy.value) return
  ...
  } catch { /* 请求失败时保留表单，拦截器已提示。 */ } finally { catDialog.saving = false }
}
async function onCatDel(row) {
  if (busy.value) return
  busy.value = true
  try {
    await ElMessageBox.confirm(`删除分类「${row.name}」？其下工具也会删除。`, '确认', { type: 'warning', confirmButtonText: '确认删除', cancelButtonText: '取消' })
    if (disposed) return
    await deleteCategory(row.id)
    if (disposed) return
    if (toolCatFilter.value === row.id) toolCatFilter.value = null
    ElMessage.success('已删除'); await loadCats(); await loadTools()
...
async function submitTool() {
  if (toolDialog.saving || busy.value) return
  ...
  } catch { /* 请求失败时保留表单，拦截器已提示。 */ } finally { toolDialog.saving = false }
}
async function onToolDel(row) {
  if (busy.value) return
  busy.value = true
  try {
    await ElMessageBox.confirm(`删除工具「${row.name}」？`, '确认', { type: 'warning', confirmButtonText: '确认删除', cancelButtonText: '取消' })
    if (disposed) return
    await deleteTool(row.id)
    if (disposed) return
    ElMessage.success('已删除'); await loadTools()
  } catch { /* 用户取消或请求拦截器已提示。 */ } finally { busy.value = false }
}
async function onToggle(row) {
  ...
  try {
    ...
    if (disposed) return
    ElMessage.success('已切换'); await loadTools()
  } catch { /* 请求拦截器已提示。 */ } finally { busy.value = false }
}
</script>
