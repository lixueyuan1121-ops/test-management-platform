...
export const stopEvalRun = (runId) => http.post(`/eval-queue/${runId}/stop`)
export const listEvalRunAttempts = (runId) => http.get(`/eval-queue/${runId}/attempts`)
...
export const listEvalRuns = (projectId, batchId) => http.get('/eval-queue/history', { params: { project_id: projectId, ...(batchId ? { batch_id: batchId } : {}) } })
...
// 推送勾选的测评结果到 multica；payload: { project_id, run_ids, batch_id? }
...
export const getExecRun = id => http.get(`/exec-queue/${id}`)
