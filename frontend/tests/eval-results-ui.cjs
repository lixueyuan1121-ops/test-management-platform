...
(async () => {
  ...
  try {
    ...
    await page.route(url => url.pathname.startsWith('/api/'), async route => {
      ...
      if (path.endsWith('/101/attempts')) data = [{ id: 1, attempt: 1, archived_at: '2026-09-12T10:00:00', status: 'failed', answer: '重试前的回答', review_note: '重试前的复核意见', raw_message: { old: true }, trace: '/uploads/eval_traces/101-old.json' }];
    ...
    await inspector.getByRole('tab', { name: '重试历史' }).click();
    await inspector.getByRole('button', { name: /第 1 次执行/ }).click();
    await inspector.getByText('重试前的回答', { exact: true }).waitFor();
    await inspector.getByText('重试前的复核意见', { exact: true }).waitFor();
    assert.equal(await inspector.getByRole('link', { name: '查看当次采集记录' }).getAttribute('href'), `${new URL(page.url()).origin}/uploads/eval_traces/101-old.json`);
    await page.waitForTimeout(350);
    await page.screenshot({ path: '/tmp/eval-retry-history.png', fullPage: true });
