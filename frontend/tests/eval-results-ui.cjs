...
(async () => {
  ...
  try {
    ...
    const rows = [
      { run_id: 101, eval_query_id: 1, status: 'judged', verdict: 'pass', payload: { title: '文件分析', prompt: '分析上传的文件', conversation_group: 'g1', turn_index: 0 } },
      ...
      { run_id: 104, eval_query_id: 4, status: 'failed', reason: '测试错误', payload: { title: '失败用例' } },
      { run_id: 105, eval_query_id: 5, status: 'judged', verdict: 'pass', pushed_multica: true, payload: { title: '已推送用例' } },
    ...
    console.log('PASS results/analysis views, chart rendering/resizing, multi-turn selection, push payload, select-all exclusions, clear, export dialog, mobile layout');
