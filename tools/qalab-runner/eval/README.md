...
平台接入模式会从当前回答的图片链接及内嵌图片中采集 PNG/JPG/WebP/GIF 实际文件，即使没有配置产物校验规则。
其他文件仍按产物校验规则采集。下载失败会记录采集原因，不把文件名、下载链接或回答中的“已完成”当作内容核验结果。
平台判定时校验本次执行的文件哈希、解码图片，向支持视觉的判定引擎附带等比预览及原图宽高/比例。
单次最多传6张图片，多帧图片仅取首帧；未取得图片或引擎无视觉能力时保留证据不足。
历史记录如果只保存了链接，须重新执行采集图片后再判定，单纯重判不会自动下载历史链接。
...
纳米 Work 的双选项单选浮层（包括受保护操作确认）按组件结构识别，固定选择第 1 项，核对第 2 项已取消后提交，
不依赖标题、描述、选项及提交按钮文案。具体授权范围以产品第 1 项为准，执行器不修改全局安全设置。
提交后等待卡片关闭，不重复点击；选中失败、按钮不可用或提交后 15 秒仍未关闭时报告 `NAMI_PROTECTED_OPERATION`。
启动检测和新建对话前都会检查浮层：未提交或未关闭时不能继续新建对话，处理失败也不能通过重试/刷新跳过。
内联普通反问、浮层多选、填空及三项以上问题沿用原有反问流程。
同一任务可多次出现该卡片，每个新请求都会重新选择并提交；等待交互时不会把已有回答误判为任务完成。
若设备仍停在该弹窗，先核对执行器日志是否出现“受保护操作浮层：已选择第 1 项”。平台接入模式启动时会检查更新，
