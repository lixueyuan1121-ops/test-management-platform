...
纳米 Work 的双选项单选浮层（包括受保护操作确认）按组件结构识别，固定选择第 2 项，核对第 1 项已取消后提交，
不依赖标题、描述、选项及提交按钮文案。具体授权范围以产品第 2 项为准，执行器不修改全局安全设置。
...
若设备仍停在该弹窗，先核对执行器日志是否出现“受保护操作浮层：已选择第 2 项”。平台接入模式启动时会检查更新，
