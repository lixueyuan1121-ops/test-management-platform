...
module.exports = {
  ...
  platform: {
    ...
    // 普通反问保留已选项，否则选第一项；文本框按原需求继续，缺失信息要求标注假设。
    // 同一任务的多次/分页反问逐次处理并记录证据，提交未关闭时不重复点击。
  ...
  execution: {
    ...
    dialogOptions: {
      ...
      chatMode: '',       // 对话模式：边想边做 / 先规划，再执行 / 盯住目标做到底（2026-08 页面文案；旧版"智能模式/计划模式/目标模式"已改版，逗号为中文逗号须原样）
